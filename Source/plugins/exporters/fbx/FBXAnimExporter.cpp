...
void FBXAnimExporter::run()
{
  ...
  QString file_name = QString("%1_%5/%2_%3_%4.fbx").arg(srcfileName).arg(justfileName).arg(animationName).arg(curAnimation.Index).arg("Animations");
  if (useAltNaming)
  {
    file_name = QString("%1_%5/%2_%4_%3.fbx").arg(srcfileName).arg(justfileName).arg(animationName).arg(curAnimation.Index).arg("Animations");
  }
